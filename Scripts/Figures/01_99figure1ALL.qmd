...
aoiDist <- inner_join(beijing_aoi,df_classify, by = join_by(ID == id), relationship = "many-to-many")
rm(df_classify)
...
aoiDist <- left_join(aoiDist,mp,by = join_by(ID == id))
...
beijing_aoi <- ggplot(data = beijing)+
  geom_sf(fill = "white") +
...
p1 <- ggarrange(Pbeijing_aoi, beijing_mp, ncol = 2, hjust = c(-1, -1), labels = c("a", "b"),widths = 12,heights = 6,font.label = list(size = 20, color = "black", face = "bold", family = NULL))
p2 <- ggarrange(beijing_aoi_a, beijing_aoi_b, beijing_aoi_c, nrow = 1, labels = c("c","d","e"),widths = 12,heights = 2,font.label = list(size = 20, color = "black", face = "bold", family = NULL))
...
# ggsave("~/ClimBehav/results/plots/figure1.eps",plot = p, device = "eps", width =12, height =8)
# ggsave("~/ClimBehav/results/plots/figure1.pdf",plot = p, device = "pdf", width =12, height =8)
```
